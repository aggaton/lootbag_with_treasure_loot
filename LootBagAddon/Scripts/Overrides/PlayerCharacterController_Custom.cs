        ...
        public override void UpdateInput()
        {
            ...
            if (InputManager.GetButtonDown("Loot"))
            {
                if (EnemyEntityDetector.characters.Count > 0)
                {
                    ...
                    {
                        if ((characterEntity.IsDead() || characterEntity.isLootable) && characterEntity.useLootBag && characterEntity.LootBag.Count > 0)
                        {
            ...
            if (!isPointerOverUI && (getMouse || getMouseUp))
            {
                ...
                for (int tempCounter = 0; tempCounter < tempCount; ++tempCounter)
                {
                    ...
                    else if (mouseUpOnTarget)
                    {
                        ...
                        if (targetCharacterEntity && (targetCharacterEntity.IsDead() || targetCharacterEntity.isLootable) && targetCharacterEntity.useLootBag)
                        {
